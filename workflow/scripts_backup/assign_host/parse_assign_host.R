...
print("open and format virus metadata")
...
##OpenDB
openDB<- fread(openDB_p) 

orgs_to_genus<- setNames(openDB$genus, openDB$SPACER_ID)
orgs_to_sp<- setNames(openDB$species, openDB$SPACER_ID)

sp_len<- c(setNames(spacers_mtdata$spacer_len, spacers_mtdata$SPACER_ID), setNames(openDB$SPACER_LENGTH, openDB$SPACER_ID))
...
blastout<- fread(blastout_p) %>%
  ...
  mutate(genus=get_mtdata(mtdata_path=clust_info_p,
         ...
         genus=ifelse(is.na(genus), orgs_to_genus[SPACER_ID], genus),
         id=get_mtdata(mtdata_path=clust_info_p,
                       "id", corr="genome", bacteria),
         id=ifelse(is.na(id), orgs_to_sp[SPACER_ID], id),
         true_mm=spacer_len - alignement_length + mismatch,
         origin=ifelse(grepl(".fna|.fa", SPACER_ID), "mine", "openDB")) %>%
  filter(abs(true_mm)<3)%>%
  filter(!(origin=="openDB" & genus %in% get_core())) # remove core bacteria matches from openDB because will not be used for PBIN
