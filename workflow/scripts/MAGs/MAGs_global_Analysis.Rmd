...
recovered_MAGs_plt<- ggplot(gtdb_comp, aes(x=factor(sample, levels = sam_levs), y=n, fill=factor(genus, levels = levs)))+
  ...
  geom_segment(aes(x = 0.5, xend = 49.5, y = -0.5, yend = -0.5),
                   size=10, col="lightpink2") +
  geom_segment(aes(x = 49.5, xend = 73.5, y = -0.5, yend = -0.5),
               size=10, col="lightcyan2") +
  annotate("text", x = 25, y = -0.5, label = "Switzerland", face="bold") +
  annotate("text", x = 61, y = -0.5, label = "Japan", face="bold") +
...
fancy_heatmap<- function(genus=""){
  ...
  col_vec = c("seashell2",magma(5, begin = 0.2, end = 1, direction = -1))
