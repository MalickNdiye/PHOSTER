...
# Mapping stats
```{r}
mapping_info<- read.csv("../../../results/inStrain/profile_B/summary_data/all_mapping_info.tsv", header=T, sep="\t")%>%
...
genome_info<-  read.csv("../../../results/inStrain/profile_B/summary_data/all_genome_info.tsv", header=T, sep="\t")
...
ggsave(file.path(fig_dir, "genera_relab_plt.svg"), genera_relab_plt,units = "mm", height = 210, width = 297, dpi=600)
```
...
put_sp_prop_plt<- ggplot(genome_plt, aes(x=factor(sample,
                                                  levels=mixedsort(unique(sample))),
                                         y=freq_inG, 
                                         fill=id))+
  geom_bar(stat = "identity", position = "stack", color = "black" , size = 0.2)+
  # geom_segment(aes(x = 0.5, xend = 49.5, y = -0.05, yend = -0.05),
  #                  size=10, col="lightpink2") +
  # geom_segment(aes(x = 49.5, xend = 73.5, y = -0.05, yend = -0.05),
  #              size=10, col="lightcyan2") +
  # annotate("text", x = 25, y = -0.05, label = "Switzerland", face="bold") +
  # annotate("text", x = 61, y = -0.05, label = "Japan", face="bold") +
...
plot_fancy_NMDS<- function(target_genus=genome_info_filt_norm$genus, plots=F){
  ...
  div_mtdata<- genome_info_filt_norm %>% 
    ...
    arrange(sample) %>%
    mutate(sam_name=as.numeric(gsub("B", "", sample)),
         colony=factor(ifelse(sam_name<23, 5, 8))) %>% 
    ungroup() %>% 
    dplyr::select(id,sample,  colony, norm_reads) %>%
    pivot_wider(names_from = id,  values_from=norm_reads, values_fill=0)
  ...
  fit<- adonis2(div_matrix~colony, data=div_mtdata, permutations = 999, method="bray")
  org_r2<- fit$R2[1]
  print(fit)
...
gene_to_genome<- gene_to_genome[unique(names(gene_to_genome))] # TODO write a parser for this
```
...
rownames(motupan_df)<- 1:nrow(motupan_df) # TODO add to parser
```
...
all_core<- sog_genomes %>% 
  ...
  mutate("id"= get_mtdata(mtdata_path=clust_filt_p, "id", corr="Bin.Id", genome)) # TODO add to parser
```
...
all_genes_df_filt<- all_genes_df %>%
  ...
  mutate(origin=ifelse(as.numeric(gsub("B", "", sample))>55, "Japan", "Switzerland")) # TODO add to parser
```

```{r}
library(data.table)
fwrite(all_genes_df_filt, "../../../results/tmp_results/genes_df_filt.tsv", 
...
ggsave("../../../Figures/230324_DMF_FriSeminar/core_vs_accessory_genes.png",
       core_vs_acc_motupan_plt,
       units = "in",
       dpi = 900,
       width = 8.3,
       height = 11.7)

ggsave("../../../Figures/230324_DMF_FriSeminar/SNVperc_mOTUpan.png",
       snv_motupan,
       units = "in",
       dpi = 900,
       width = 8.3,
