...
mapping_plt<- ggplot(mapping_info_m, aes(x=factor(sample, levels = mixedsort(unique(sample))), y=value, fill=factor(variable, levels = id_vars)))+
  geom_bar(stat="identity", position = position_dodge())+
  scale_fill_manual(values=cols)+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1), legend.position = "top"); mapping_plt
```
...
snv_motupan<- ggplot(motupan_core, aes(x=id, y=perc_snv, fill=genus))+
  ...
  theme_classic()+
  theme(axis.text.x=element_text(angle = 90, vjust = 1, hjust=1, size=5, face="bold"))+
...
```{r}
library(svglite)
fig_../../../Figures/bacteria_community_analysis
...
ggsave(paste(fig_dir, "/snv_percentage_orthofinder", sep=""),  snv_ogf, units ="mm", width = 500, height = 310, dpi=300)
